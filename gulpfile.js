...
const SRC_PATH = "src";
const SRC_HOST = "local.sandbox.net";
 ...
 */
var logLevel = args.logLevel || 'info';
 ...
 */
gulp.task('watch', ['clean', 'browser-sync', 'sass-dist', 'test-js'], function() {
	buildMode = BUILD_MODE_DEV;
	gulp.watch(SRC_PATH + "/_/sass/**/*.scss", ['sass-dev']);
	gulp.watch([SRC_PATH + "/**/*.+(html|php|inc)","!" + SRC_PATH + "/_/bower/**/*.*"]).on('change', proxyServer.reload);
	//gulp.watch([SRC_PATH + "/**/*.js","!" + SRC_PATH + "/_/bower/**/*.*"], ['lint-js']).on('change', proxyServer.reload);
	gulp.watch([SRC_PATH + "/**/*.js","!" + SRC_PATH + "/_/bower/**/*.*"]).on('change', proxyServer.reload);
});
...
/**
 * The test-js task is responsible for running unit tests
 * on javascript files named *.test.js
 *
 * Currently, we are writing tests with Jasmine.
 */
gulp.task('test-js', function(done) {
	...
	var karmaServer = new karma.Server({
		...
		if (err === 0) {
			...
			handleError(context, 'info', true, "Quitting Karma... " + err);
		}
	}).start();
